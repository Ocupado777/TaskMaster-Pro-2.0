...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname.startsWith('/api/audio-file/')) {
      const filename = url.pathname.split('/').pop();
      const file = join(uploadsDir, filename);
      if (!filename || filename.includes('..') || !existsSync(file)) return send(response, 404, { message: 'Audio no encontrado' });
      response.writeHead(200, { 'Content-Type': 'audio/webm', 'Access-Control-Allow-Origin': '*' }); return response.end(readFileSync(file));
    }
