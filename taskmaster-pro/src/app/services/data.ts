...
const API_URL = (globalThis as typeof globalThis & { __TASKMASTER_API__?: string }).__TASKMASTER_API__ || 'http://localhost:3000/api';
...
export class DataService {
  ...
  enviarMensaje(contacto: string, texto: string, audioUrl = '') {
    const emisor = this.usuario()?.usuario;
    if (!emisor || (!texto.trim() && !audioUrl)) return;
    const mensajes = this.getMensajesChat();
    const mensaje = { id: Date.now(), emisor, receptor: contacto, texto: texto.trim(), fecha: new Date().toISOString(), audioUrl };
  ...
  async enviarAudio(blob: Blob, nombre: string): Promise<string | null> {
    ...
    try {
      const respuesta = await firstValueFrom(this.http.post<{ audioUrl: string }>(`${API_URL}/audio/upload`, { data, name: nombre }, { headers: this.authHeaders }));
      return `${API_URL.replace('/api', '')}${respuesta.audioUrl}`;
    } catch { return null; }
  }
